...
{
	char BEntrada[TAM_BUFFER_ENTRADA], BSalida[TAM_BUFFER_ENTRADA];	// Reserva para buffer de entrada
	DWORD Leidos, Escritos;
	BOOL ResOperacion;
	HANDLE hPipe;	// Handle para una instancia del pipe. 

	hPipe = CreateNamedPipe(pNombreTuberia,	//Nombre
	...
	while (TRUE)
	{
		...
		BEntrada[Leidos] = '\0';
		printf("Mensaje Recibido:%s", BEntrada);
		if (BEntrada[0] == 'U') {
			LCMapString(LOCALE_USER_DEFAULT, LCMAP_UPPERCASE, BEntrada, Leidos, BSalida, Leidos);
		}
		else if (BEntrada[0] == 'L') {
			LCMapString(LOCALE_USER_DEFAULT, LCMAP_LOWERCASE, BEntrada, Leidos, BSalida, Leidos);
		}
		else {
			for (int i = 0; i < Leidos; i++) {
				BSalida[i] = BEntrada[i];
			}
		}

		ResOperacion = WriteFile(hPipe, BSalida, Leidos, &Escritos, 0);
